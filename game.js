...
images.forEach((num, index) => {
  ...
  // 数字をテキストとして設定（1と10は画像なので空にする）
  if (num === 1) {
    cardBack.textContent = ''; // 1カードは画像なのでテキストなし
  } else if (num === 10) {
    cardBack.textContent = ''; // 10カードは画像なのでテキストなし
  } else {
    cardBack.textContent = num; // その他は数字を表示
  }
...
function executeShuffle() {
  ...
  cardData.forEach((item, index) => {
    ...
    // data-number属性を新しい数字に設定（これによりCSSで背景色が自動的に変わる）
    ...
    setTimeout(() => {
      ...
      allCards.forEach(card => {
        if (!card.classList.contains('matched')) {
          ...
          // 裏側のデザインを復元（CSSデフォルトデザインを使用）
          const cardBack = card.querySelector('.card-back');
          if (cardBack) {
            // CSSのデフォルトデザインを適用
            cardBack.style.background = '';
            cardBack.style.color = '';
            cardBack.style.fontSize = '';
            cardBack.style.fontWeight = '';
            cardBack.style.fontFamily = '';
            cardBack.style.display = '';
            cardBack.style.alignItems = '';
            cardBack.style.justifyContent = '';
            
            // data-number属性に基づいて内容を設定
            const dataNumber = cardBack.getAttribute('data-number');
            if (dataNumber === '1') {
              // 1カードはCSSで画像が設定されるのでテキストのみクリア
              cardBack.textContent = '';
            } else if (dataNumber === '10') {
              // 10カードはCSSでミリちゃん画像が設定されるのでテキストのみクリア
              cardBack.textContent = '';
            } else if (dataNumber) {
              // その他の数字カード
