...
function showGetMessage(number) {
  ...
  if (number === 1) {
    ...
    getMessage.innerHTML = '<div style="text-align: center;"><span style="color: #63a4a4;">雨夜リズ</span><br><span style="font-size: 0.8em;">ゲット！</span></div>';
  } else if (number === 8) {
    getMessage.innerHTML = '<div style="text-align: center;"><span style="color: #DFB5FF;">眠雲ツクリ</span><br><span style="font-size: 0.8em;">ゲット！</span></div>';
  } else if (number === 9) {
...
images.forEach((num, index) => {
  ...
  if (num === 1) {
    ...
    // ミリカードにミリちゃんの画像を設定
    cardBack.textContent = ''; // ミリマークを非表示（先にクリア）
    const img = document.createElement("img");
    img.src = "./images/ミリちゃん_N.png";
    img.alt = "ミリちゃん通常";
    img.style.width = "85%";
    img.style.height = "85%";
    img.style.objectFit = "contain";
    ...
    img.onerror = function() {
      console.error('ミリちゃん_N.pngの読み込みに失敗しました');
      // 代替テキストを表示
      cardBack.textContent = 'ミリ';
    ...
    cardBack.setAttribute('data-number', num.toString());
  }
  ...
  card.onclick = () => {
    ...
    if (cardBack.textContent === '10' || cardBack.textContent === '×') {
        ...
        setTimeout(() => {
          ...
          if (remainingCardsAfterOut.length === 2 && remainingXCardsAfterOut.length === 2) {
            ...
            setTimeout(() => {
              ...
              // 2秒後に結果を表示
              setTimeout(() => {
                showGeneralMessage(`${player1Score}vs${player2Score}\n${player1Score > player2Score ? "甘狼このみ 勝ち！" : player2Score > player1Score ? "音ノ乃のの 勝ち！" : "引き分け！"}`, 0);
      ...
      return; // 10カードの場合はここで処理終了
    }

    card.classList.add("open");
    playSound('flip'); // カードめくり音
    
    // 10カード（ミリカード）を引いた場合の処理
    const currentCardBack = card.querySelector('.card-back');
    const currentCardNumber = currentCardBack.getAttribute('data-number');
    if (currentCardNumber === '10') {
      // 0.5秒遅らせてアウトメッセージを中央に2秒間表示
      setTimeout(() => {
        const outMessage = document.getElementById('out-message');
        const miriImageMessage = document.getElementById('miri-image-message');
        outMessage.classList.remove('hidden');
        miriImageMessage.classList.remove('hidden');
        
        // ミリカードを引いたプレイヤーに泣き顔を表示
        if (currentPlayer === 1) {
          showPlayer1SadFace();
        } else {
          showPlayer2SadFace();
        ...
        setTimeout(() => {
          outMessage.classList.add('hidden');
          miriImageMessage.classList.add('hidden');
          card.classList.remove("open");
          // firstCardがあればそれも裏返す
          if (firstCard) {
            firstCard.classList.remove("open");
          ...
          // アンコールが発動されている場合は手番を継続、そうでなければ手番を終了
          if (extraTurn) {
            extraTurn = false; // アンコールを消費
            lock = false; // ロックを解除して続行可能に
            ...
            console.log('ミリカード引いた後もアンコールで手番継続');
          } else {
            // ミリカードを引いた場合はカードの手番だけ終了し、待機中スキルを発動させる
            console.log('ミリカード引いた後の待機中スキル発動');
      ...
      if (firstCardNumber === '10' || secondCardNumber === '10') {
        ...
        setTimeout(() => {
          ...
          if (extraTurn) {
            ...
            showGeneralMessage('アンコール！もう一度プレイ！');
            console.log('ミリカードペア判定後もアンコールで手番継続');
          } else {
            // ミリカードを引いた場合はカードの手番だけ終了し、待機中スキルは発動させる
            console.log('ミリカードペア判定後の待機中スキル発動');
...
function showGameResult() {
  ...
  if (player1Score > player2Score) {
    winner = 'Player1';
    resultMessage = `甘狼このみ 勝利！`;
  } else if (player2Score > player1Score) {
    winner = 'Player2';
    resultMessage = `音ノ乃のの 勝利！`;
  } else {
    resultMessage = `引き分け！`;
  }
...
function executeShuffle() {
  ...
  shuffleTargets.forEach(item => {
    ...
    // 表面を完全に隠す
    const cardFront = item.card.querySelector('.card-front');
    if (cardFront) {
      cardFront.style.background = 'linear-gradient(to bottom, #8b9dff 0%, #a78bfa 100%)';
      cardFront.textContent = ''; // アルファベットを非表示
      cardFront.style.visibility = 'visible'; // 確実に表示
      cardFront.style.zIndex = '10'; // 前面に
      cardFront.style.position = 'absolute'; // 絶対位置で完全覆う
    ...
    const cardBack = item.card.querySelector('.card-back');
    if (cardBack) {
      cardBack.style.background = 'linear-gradient(to bottom, #8b9dff 0%, #a78bfa 100%)';
      cardBack.textContent = ''; // 数字や画像を非表示
      cardBack.style.visibility = 'visible'; // 確実に表示
      cardBack.style.zIndex = '5'; // 背面に
    }
    
    // カード全体の背面が見えないように設定
    item.card.style.backfaceVisibility = 'hidden';
    item.card.style.transformStyle = 'preserve-3d';
    item.card.style.perspective = '1000px';
    item.card.style.overflow = 'hidden'; // はみ出し防止
  });
  ...
  setTimeout(() => {
    ...
    for (let pos = 0; pos < allPositions.length; pos++) {
      ...
      if (targetItem) {
        ...
        // 裏面も青紫色で確実に覆う
        const cardBack = targetItem.card.querySelector('.card-back');
        if (cardBack) {
          cardBack.style.background = 'linear-gradient(to bottom, #8b9dff 0%, #a78bfa 100%)';
          cardBack.style.visibility = 'visible'; // 確実に表示
          cardBack.textContent = ''; // 数字を一時的に非表示
        }
        
        // 再配置時も背面が見えないように設定
        targetItem.card.style.backfaceVisibility = 'hidden';
        ...
        // 裏面も青紫色で確実に覆う
        const cardBack = originalCard.querySelector('.card-back');
        if (cardBack && !originalCard.classList.contains('matched')) {
          cardBack.style.background = 'linear-gradient(to bottom, #8b9dff 0%, #a78bfa 100%)';
          cardBack.style.visibility = 'visible'; // 確実に表示
        }
        
        // 再配置時も背面が見えないように設定
        originalCard.style.backfaceVisibility = 'hidden';
      }
    }
    
    // 配置直後に全カードの青紫色を維持（アニメーション完了まで）
    setTimeout(() => {
      const allCards = document.querySelectorAll('.card');
      allCards.forEach(card => {
        if (!card.classList.contains('matched')) {
          ...
          if (cardBack) {
            ...
            cardBack.style.zIndex = '5'; // 背面に
            const dataNumber = cardBack.getAttribute('data-number');
            if (dataNumber) {
              if (dataNumber === '10') {
                cardBack.textContent = '×';
              } else {
                cardBack.textContent = dataNumber;
              }
            }
          }
          ...
          card.style.overflow = 'hidden'; // はみ出し防止
        }
      });
    ...
    // アニメーションを元に戻す
    setTimeout(() => {
      // シャッフル対象カードのデザインを元に戻す
      shuffleTargets.forEach(item => {
        item.card.style.transform = '';
        item.card.style.opacity = '';
        item.card.style.transition = ''; // transitionを元に戻す
        item.card.style.backfaceVisibility = ''; // backfaceVisibilityを元に戻す
        item.card.style.transformStyle = ''; // transformStyleを元に戻す
        item.card.style.perspective = ''; // perspectiveを元に戻す
        item.card.style.overflow = ''; // overflowを元に戻す
        
        // 表面のデザインを元に戻す
        const cardFront = item.card.querySelector('.card-front');
        if (cardFront) {
          cardFront.style.background = ''; // CSSのデフォルトデザインに戻す
          cardFront.style.visibility = ''; // visibilityを元に戻す
          cardFront.style.zIndex = ''; // zIndexを元に戻す
          cardFront.style.position = ''; // positionを元に戻す
          cardFront.style.top = ''; // topを元に戻す
          cardFront.style.left = ''; // leftを元に戻す
          cardFront.style.width = ''; // widthを元に戻す
          cardFront.style.height = ''; // heightを元に戻す
          // アルファベットは再配置時に設定済み
        }
        ...
        const cardBack = item.card.querySelector('.card-back');
        if (cardBack) {
          cardBack.style.background = ''; // CSSのデフォルトデザインに戻す
          cardBack.style.visibility = ''; // visibilityを元に戻す
          cardBack.style.zIndex = ''; // zIndexを元に戻す
          // 裏面の数字や画像を再設定
          const dataNumber = cardBack.getAttribute('data-number');
          if (dataNumber) {
            if (dataNumber === '10') {
              // ミリカードの場合は画像を復元
              cardBack.textContent = ''; // テキストをクリア
              const img = document.createElement("img");
              img.src = "./images/ミリちゃん_N.png";
              img.alt = "ミリちゃん通常";
              img.style.width = "85%";
              ...
              // 画像読み込み完了を待ってDOMに追加
              img.onload = function() {
                console.log('ミリちゃん_N.pngの読み込み完了');
                cardBack.appendChild(img);
              };
              ...
              // 画像読み込みを開始
              img.style.display = 'none'; // 一時的に非表示
              cardBack.appendChild(img);
              img.style.display = ''; // 表示を戻す
            } else {
              cardBack.textContent = dataNumber;
            }
          }
      ...
      // シャッフル対象外のカードのデザインも元に戻す
      cards.forEach((card, index) => {
        if (!shuffleTargets.includes(card)) {
          const cardFront = card.querySelector('.card-front');
          if (cardFront) {
            cardFront.style.background = ''; // CSSのデフォルトデザインに戻す
            cardFront.style.visibility = ''; // visibilityを元に戻す
          ...
          if (cardBack) {
            ...
            if (dataNumber === '10') {
              ...
              // 画像読み込み完了を待ってDOMに追加
              img.onload = function() {
                console.log('ミリちゃん_N.pngの読み込み完了（シャッフル対象外）');
                cardBack.appendChild(img);
              };
              
              // 画像読み込みエラー処理
              img.onerror = function() {
                console.error('ミリちゃん_N.pngの読み込みに失敗しました（シャッフル対象外）');
                // 代替テキストを表示
                cardBack.textContent = 'ミリ';
                cardBack.style.color = 'white';
                cardBack.style.fontSize = '24px';
          ...
          card.style.backfaceVisibility = ''; // backfaceVisibilityを元に戻す
          card.style.transformStyle = ''; // transformStyleを元に戻す
          card.style.perspective = ''; // perspectiveを元に戻す
          card.style.overflow = ''; // overflowを元に戻す
        }
      });
      ...
      // 自分がリミックスを発動したら笑顔を表示
      const shufflePlayer = currentPlayer === 1 ? 2 : 1; // 手番交代後のプレイヤー
      if (shufflePlayer === 1) {
        showPlayer1DecorationCard();
      } else {
        showPlayer2DecorationCard();
      ...
      const opponentPlayer = shufflePlayer === 1 ? 2 : 1;
      if (opponentPlayer === 1) {
        showPlayer1SadFace();
      } else {
        showPlayer2SadFace();
